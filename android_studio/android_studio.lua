...
function m.generate_workspace_settings(wks)
    ...
    p.x('        }')
    p.x('        mavenCentral()')
    p.x('        gradlePluginPortal()')
    p.x('    }')
    p.x('}')
    p.x('dependencyResolutionManagement {')
    p.x('    repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)')
    p.x('    repositories {')
    p.x('        google()')
    p.x('        mavenCentral()')
    p.x('    }')
...
function m.generate_cmake_lists(prj)
    p.w('# auto-generated by premake-android-studio')
    p.w('cmake_minimum_required (VERSION %s)', prj.cmakeversion)
...
function m.generate_project(prj)
    ...
    p.push('cmake {')
    for _, opt in ipairs(prj.buildoptions) do
        p.x('cppFlags += "%s"', opt)
    end
    p.pop('}') -- cmake
    ...
    p.x('')
    p.push('buildTypes {')
    for _, val in pairs(prj.buildtypesconfiguration) do
        if val[1] == "debug" or val[1] == "release" then
            p.push('%s {', val[1])
        else
            p.push('create("%s") {', val[1])
        end

        for _, row in pairs(val[2]) do
            p.x(row)
        end
    ...
    -- compile options
    p.x('')
    p.push('compileOptions {')
    for _, val in ipairs(prj.compileoptions) do
        p.x(val)
    end
    p.pop('}') -- compile options

    -- kotlin options
    p.x('')
    p.push('kotlinOptions {')
    for _, val in ipairs(prj.kotlinoptions) do
        p.x(val)
    end
    p.pop('}') -- kotlin options

    -- build features
    p.x('')
    p.push('buildFeatures {')
    for _, val in ipairs(prj.buildfeatures) do
        p.x(val)
    end
    p.pop('}') -- build features
    ...
    -- flavors
    p.x('')
    if prj.platforms ~= nil then
        p.x('flavorDimensions += listOf("Build")')
        p.push('productFlavors {')
        for _, val in pairs(prj.flavorsconfiguration) do
            p.x('')
            p.push('create("%s") {', val[1])    
            p.x('dimension = "Build"')
            for _, row in pairs(val[2]) do
